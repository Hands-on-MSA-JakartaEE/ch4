...
// Model for FootballPlayer entity
    models.FootballPlayer = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/footballplayer/",
        defaults: {
            surname: "",
            name: "",
            team: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/footballplayer/';
            }
    ...
    // Collection class for FootballPlayer entities
    models.FootballPlayerCollection = Backbone.Collection.extend({
        model: models.FootballPlayer,
        url: "http://localhost:8080/footballplayer/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.FootballPlayerCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-footballplayer-list-itemis template identifier for item
                        templateName: '#tpl-footballplayer-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.footballplayer = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.footballplayer,
                    // tpl-footballplayer-update-details is template identifier for chosen model element
                    templateName: '#tpl-footballplayer-update-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.FootballPlayer();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-footballplayer-details is a template identifier for chosen model element
                templateName: '#tpl-footballplayer-details',
