...
// Model for FootballManager entity
    models.FootballManager = Backbone.Model.extend({
        urlRoot: "http://localhost:8180/footballmanager/",
        defaults: {
            surname: "",
            name: "",
            nickname: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8180/footballmanager/';
            }
    ...
    // Collection class for FootballManager entities
    models.FootballManagerCollection = Backbone.Collection.extend({
        model: models.FootballManager,
        url: "http://localhost:8180/footballmanager/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.FootballManagerCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-footballmanager-list-itemis template identifier for item
                        templateName: '#tpl-footballmanager-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.footballmanager = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.footballmanager,
                    // tpl-footballmanager-details is template identifier for chosen model element
                    templateName: '#tpl-footballmanager-update-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.FootballManager();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-footballmanager-details is a template identifier for chosen model element
                templateName: '#tpl-footballmanager-details',
